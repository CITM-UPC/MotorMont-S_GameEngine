...
void GraphicObject::draw() const {
    glPushMatrix();
    glMultMatrixf(_transform.data()); // Changed to `glMultMatrixf` to match float data type
...
BoundingBox GraphicObject::localBoundingBox() const {
    if (!children().empty()) {
        ...
        return _mesh_ptr ? _mesh_ptr->boundingBox() : BoundingBox();
    }
}
...
BoundingBox GraphicObject::worldBoundingBox() const {
    ...
    for (const auto& child : children()) {
        bbox = bbox + child.worldBoundingBox();
    }
    return bbox;
}
